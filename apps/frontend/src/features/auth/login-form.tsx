...
export function LoginForm() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const loggedIn = loginDemo(username, password);
      if (!loggedIn) throw new Error('Usuario o contraseña incorrectos.');
      const requested = params.get('next');
      const allowedRequested = loggedIn.role === 'CASHIER' && requested?.startsWith('/');
      router.replace(loggedIn.role === 'KITCHEN' ? '/kitchen' : allowedRequested ? requested! : destination === 'kitchen' ? '/kitchen' : '/pos');
  ...
  return <main className="login-shell">
    ...
    <section className="login-access-panel">
      <form onSubmit={submit} className="login-form" noValidate>
        ...
        <label>Usuario<span className="login-input"><FiUser size={18} /><input type="text" autoComplete="username" value={username} onChange={(event) => setUsername(event.target.value)} placeholder="Ingresa tu usuario" minLength={3} required /></span></label>
